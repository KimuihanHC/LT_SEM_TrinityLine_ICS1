...
IMPLEMENT_DYNAMIC(CList_ServerConfig, CListCtrl)

CList_ServerConfig::CList_ServerConfig()
{
...
BEGIN_MESSAGE_MAP(CList_ServerConfig, CListCtrl)
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_NOTIFY_REFLECT(NM_CLICK,	&CList_ServerConfig::OnNMClick)
	ON_WM_MOUSEWHEEL()
END_MESSAGE_MAP()
...
// Desc.		:
//=============================================================================
int CList_ServerConfig::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::OnSize(UINT nType, int cx, int cy)
{
...
// Desc.		:
//=============================================================================
BOOL CList_ServerConfig::PreCreateWindow(CREATESTRUCT& cs)
{
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::OnNMClick(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::InitHeader()
{
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Reset_Number()
{
	CString szText;
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Select_SelectItem(__in int nItem)
{
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Set_ServerData(__in int nItem, __in const CConfig_Svr* IN_pEquipment)
{
	ASSERT(GetSafeHwnd());
	...
	SetItemText(nItem, TSH_EqpType, g_szServerUIName[IN_pEquipment->Get_SvrType_UI()]);

	// Equipment id
	SetItemText(nItem, TSH_EqpId, IN_pEquipment->Get_ServerId());
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Insert_Server(__in int nItem, __in const CConfig_Svr* IN_pEquipment)
{
	ASSERT(GetSafeHwnd());
	if (NULL == IN_pEquipment)
		return;
...
void CList_ServerConfig::Add_Server(const __in CConfig_Svr* IN_pEquipment)
{
	ASSERT(GetSafeHwnd());
	if (NULL == IN_pEquipment)
		return;
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Modify_Server(__in int nItem, __in const CConfig_Svr* IN_pEquipment)
{
	ASSERT(GetSafeHwnd());
	if (NULL == IN_pEquipment)
		return;
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::OnLanguage(__in uint8_t IN_nLanguage)
{
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Set_LineInfo(__in const CConfig_Server* IN_pLineInfo)
{
	...
	if (NULL != IN_pLineInfo)
	{
		...
		//2023.02.13a uhkim 추가 UI
		for (UINT nIdx = 0; nIdx < IN_pLineInfo->GetCount(); nIdx++)
		{
			Add_Server(&(IN_pLineInfo->GetAt(nIdx)));
		}
	}
}
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Get_LineInfo(__out CConfig_Server& OUT_stLineInfo)
{
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Item_Add(__in CConfig_Svr& IN_Equipment)
{
	...
	m_stLine_Info.Svr_Add(IN_Equipment);
	Add_Server(&IN_Equipment);

	// UI 갱신
	//Check_EqpType_UpdateAlias(IN_Equipment.Get_EquipmentType());
	Check_UI_UpdateAlias(IN_Equipment.Get_SvrType_UI());
	Set_Group_Server();
}
//=============================================================================
// Method		: Item_Insert
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Item_Insert(__in CConfig_Svr& IN_Equipment)
{
	if (0 < GetSelectedCount())
	{
		...
		m_stLine_Info.Svr_Insert(iIndex, IN_Equipment);

		Insert_Server(iIndex, &IN_Equipment);
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Item_Remove()
{
	if (0 < GetSelectedCount())
	{
		POSITION nPos = GetFirstSelectedItemPosition();
		...
		UINT stSelEqpTypeUI = m_stLine_Info.GetAt(iSelIdx).Get_SvrType_UI();


		m_stLine_Info.Svr_Remove(iSelIdx);


		// UI 갱신
		//Check_EqpType_UpdateAlias(stSelEqpType);
		Check_UI_UpdateAlias(stSelEqpTypeUI);
		Set_Group_Server();
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Item_Up()
{
	if (0 < GetSelectedCount())
	{
		POSITION nPos = GetFirstSelectedItemPosition();
		...
		{
			CConfig_Svr stStep = m_stLine_Info.GetAt(iIndex);

			DeleteItem(iIndex);
			Insert_Server(iIndex - 1, &stStep);

			// Equipment Alias 체크
			//UINT stSelEqpType = m_stLine_Info.GetAt(iIndex).Get_EquipmentType();
			UINT stSelEqpTypeUI = m_stLine_Info.GetAt(iIndex).Get_SvrType_UI();

			m_stLine_Info.Svr_Remove(iIndex);
			m_stLine_Info.Svr_Insert(iIndex - 1, stStep);
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Item_Down()
{
	if (0 < GetSelectedCount())
	{
		POSITION nPos = GetFirstSelectedItemPosition();
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Item_Modify(__in CConfig_Svr& IN_Equipment)
{
	if (0 < GetSelectedCount())
	{
		POSITION nPos = GetFirstSelectedItemPosition();
		...
		// 수정
		m_stLine_Info.Svr_Modify(iIndex, IN_Equipment);
		// 삭제, 추가
		Modify_Server(iIndex, &IN_Equipment);

		// UI 갱신
		//Check_EqpType_UpdateAlias();
		Check_UI_UpdateAlias();
...
// Desc.		:
//=============================================================================
bool CList_ServerConfig::Get_Selected(__out CConfig_Svr& OUT_Equipment)
{
	if (0 < GetSelectedCount())
	{
		POSITION nPos = GetFirstSelectedItemPosition();
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Add_LineInfo(__in const CConfig_Server* IN_pLineInfo)
{
	if (NULL != IN_pLineInfo)
	{
		m_stLine_Info = *IN_pLineInfo;
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Insert_LineInfo(__in const CConfig_Server* IN_pLineInfo)
{
	...
	if (0 < GetSelectedCount())
	{
		...
		for (UINT nIdx = 0; nIdx < IN_pLineInfo->GetCount(); nIdx++)
		{
			m_stLine_Info.Svr_Insert(iIndex, IN_pLineInfo->GetAt(nIdx));

			Insert_Server(iIndex, &(IN_pLineInfo->GetAt(nIdx)));
		}
	}
	else
	{
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Clear_LineInfo()
{
	m_stLine_Info.RemoveAll();
	DeleteAllItems();
}
...
// Desc.		:
//=============================================================================
UINT CList_ServerConfig::Get_ServerCount(__in UINT nEquipmentType)
{
	return m_stLine_Info.Get_ServerCount(nEquipmentType);
}
...
// Desc.		:
//=============================================================================
void CList_ServerConfig::Check_UI_UpdateAlias(__in UINT nEqpTypeUI, __in bool bUpdateListCtrl /*= true*/)
{
	bool bReturn = true;
	...
	for (auto nIdx = 0; nIdx < m_stLine_Info.SvrList.size(); ++nIdx)
	{
		// 설비 순서 설정
		m_stLine_Info.SvrList.at(nIdx).Set_SvrOrder(nIdx);

		if (nEqpTypeUI == m_stLine_Info.SvrList.at(nIdx).Get_SvrType_UI())
		{
			++nCount;

			szValue.Format(_T("%s #%02d"), g_szServerUIName[nEqpTypeUI], nCount);

			m_stLine_Info.SvrList.at(nIdx).Set_Alias(szValue);

			// 검사 그룹내 설비 인덱스
			m_stLine_Info.SvrList.at(nIdx).Set_GroupIndex(nCount - 1);
		}
	}
...
void CList_ServerConfig::Check_UI_UpdateAlias()
{
	// 모든 설비 타입을 검색해서 Alias를 재 설정
	for (auto nIdx = 0; nIdx < SERVER_UI_MAX; ++nIdx)
	{
		Check_UI_UpdateAlias(nIdx, false);
	}
	Set_Group_Server();
...
//=============================================================================
bool CList_ServerConfig::Check_ServerID()
{
	for (auto nIdx = 0; nIdx < m_stLine_Info.GetCount(); ++nIdx)
	{
		for (auto nComp = nIdx + 1; nComp < m_stLine_Info.GetCount(); ++nComp)
		{
			if (0 == m_stLine_Info.GetAt(nIdx).Get_ServerId().Compare(m_stLine_Info.GetAt(nComp).Get_ServerId()))
			{
				return false;
			}
		}
...
bool CList_ServerConfig::Check_ServerID(__in LPCTSTR IN_szEquipmentID)
{
	for (auto nIdx = 0; nIdx < m_stLine_Info.GetCount(); ++nIdx)
	{
		if (0 == m_stLine_Info.GetAt(nIdx).Get_ServerId().Compare(IN_szEquipmentID))
		{
			return false;
		}
	}
...
//=============================================================================
bool CList_ServerConfig::Check_IPAddress()
{
	for (auto nIdx = 0; nIdx < m_stLine_Info.GetCount(); ++nIdx)
	{
		for (auto nComp = nIdx + 1; nComp < m_stLine_Info.GetCount(); ++nComp)
...
bool CList_ServerConfig::Check_IPAddress(__in DWORD IN_nIPAddress)
{
	for (auto nIdx = 0; nIdx < m_stLine_Info.GetCount(); ++nIdx)
	{
...
//=============================================================================
void CList_ServerConfig::Set_Group_Server()
{
	// 설비 타입
	...
	for (uint8_t nEqpType = SERVER_EES; nEqpType <= SERVER_EES; ++nEqpType)
	{
		nGoupCount = 0;
		...
		for (auto nIdx = 0; nIdx < m_stLine_Info.GetCount(); ++nIdx)
		{
			if (nEqpType == m_stLine_Info.GetAt(nIdx).Get_ServerType())
			{
				++nGoupCount;
			}
		}
