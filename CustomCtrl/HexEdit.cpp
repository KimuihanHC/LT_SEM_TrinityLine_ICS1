...
void CHexEdit::FormatClipboard()
{
    ...
    { 
        lptstr =(LPTSTR) GlobalLock(hglb);
		for(unsigned int i=0;i<_tcslen(lptstr);i++)
		{
		...
		memcpy(lptstrCopy, lptstr,_tcslen(lptstr));
		GlobalUnlock(hglb); 
...
LRESULT CHexEdit::OnConvert(WPARAM wParam, LPARAM lParam)
{
	...
	else
	{
		//convert dec to hex
		UINT n=0;
		_stscanf_s(text.GetBuffer(0),_T("%d"),&n);
