...
//
//=============================================================================
CButtonExProp::CButtonExProp( const CString& strName, const COleVariant& varValue, CWnd* pParentWnd, UINT nID ) : CMFCPropertyGridProperty(strName, varValue)
{
	//m_pWndList;	// Pointer to the PropertyList window
	//m_pParent;	// Parent property (NULL for top-level properties)

	m_pwndOwner	= pParentWnd;
	m_nID		= m_nID;
}

CButtonExProp::~CButtonExProp()
{

}

CWnd* CButtonExProp::CreateInPlaceEdit( CRect rectEdit, BOOL& bDefaultFormat)
{
	CButton* pWndButton = new CButton;

	//rectEdit = m_pWndList->GetListRect();	
	//rectEdit = m_Rect;
	//rectEdit = m_rectButton;
	//m_pWndInPlace->GetWindowRect(rectEdit);
	
	rectEdit.InflateRect(2,2,0,3);
	

	DWORD style=WS_VISIBLE | WS_CHILD;

	//pWndButton->Create("", style, rectEdit, m_pWndList, AFX_PROPLIST_ID_INPLACE);
	//pWndButton->Create("", style, rectEdit, m_pWndList, m_nID);
	pWndButton->Create(_T(""), style | BS_PUSHBUTTON, rectEdit, m_pwndOwner, m_nID);
	pWndButton->SetOwner(m_pwndOwner);
	
	pWndButton->MoveWindow(rectEdit.left, rectEdit.bottom + 12, rectEdit.Width(), rectEdit.Height());

	bDefaultFormat = TRUE;
	return pWndButton;
}

//  BOOL CButtonExProp::HasButton() const
//  {
//  	return TRUE;
//  }

void CButtonExProp::OnClickButton( CPoint point )
{
	ASSERT_VALID(this);
	ASSERT_VALID(m_pWndList);
	ASSERT_VALID(m_pWndInPlace);
...
//
//=============================================================================
CButtonTrProp::CButtonTrProp( const CString& strName, const COleVariant& varValue, UINT nWM, UINT nWParam) : CMFCPropertyGridProperty(strName, varValue)
{
	m_nWndMsg		= nWM;
	m_nWndMsgWParam	= nWParam;
}

//void CButtonTrProp::AdjustButtonRect()
//{
//	CMFCPropertyGridProperty::AdjustButtonRect();
//	m_rectButton.left -= m_rectButton.Width();
//}

void CButtonTrProp::OnClickButton( CPoint point )
{
	AfxGetApp()->GetMainWnd()->SendNotifyMessage (m_nWndMsg, (WPARAM)m_nWndMsgWParam, (LPARAM)0);
}

void CButtonTrProp::OnDrawValue( CDC* pDC, CRect rect )
{
	m_rectButton = rect;
...
void CButtonTrProp::OnDrawButton( CDC* pDC, CRect rectButton )
{
	CMFCToolBarButton button;
	CRect rect = rectButton;	
